...
void require_shape(const Tensor& tensor, const std::vector<size_t>& expected) {
    REQUIRE(tensor.shape() == expected);
}

void require_data(const Tensor& tensor, const std::vector<float>& expected) {
...
TEST_CASE("Gradient-check utility guards", "[autograd][gradcheck]") {
    SECTION("scalar_value enforces singleton tensors") {
        ...
        REQUIRE_THROWS_WITH(scalar_value(vector), ContainsSubstring("exactly one element"));
    }
